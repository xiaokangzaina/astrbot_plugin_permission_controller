...
function privateContactsFromConfig(config) {
  ...
  return [...userIds].map((userId) => ({
    ...
    is_configured: enabledUsers.has(userId) || reasoningMap.has(userId),
  }));
}
...
function isObjectConfigured(item, isPrivate) {
  if (item?.is_configured === true) return true;
  if (isPrivate) return Boolean(item?.private_enabled || normalizeReasoningValue(item?.reasoning_effort));
  return Boolean(item?.group_enabled || normalizeReasoningValue(item?.reasoning_effort));
}
...
function renderObjectList() {
  ...
  items.forEach((item) => {
    ...
    const configured = activePluginId === "permission"
      ? isObjectConfigured(item, isPrivate)
      : pluginStatus?.kind === "is-on";
    const button = document.createElement("button");
    button.type = "button";
    button.className = `object-item ${active ? "active" : ""} ${configured ? "is-configured" : ""}`.trim();
    ...
    if (pluginStatus) {
      ...
      statusKind = configured ? "is-on" : "is-off";
      ...
      statusKind = configured ? "is-on" : "is-neutral";
