...
支持 Web 配置页、群聊整体放行、用户-群号组合规则、私聊白名单、群聊黑名单和模型思考强度配置，并会把放行对象同步到 AstrBot 平台白名单，避免核心白名单阶段提前拦截。
...
默认表示不注入任何参数，保持模型服务商或 AstrBot provider 的原始配置。
...
- 新增模型思考强度配置，支持默认、低、中、高、超高
- 支持按群聊默认、群成员覆盖、私聊好友覆盖配置思考强度
- 配置页同步新增群聊和私聊思考强度控件
- 对 OpenAI-compatible provider 注入 `reasoning_effort`，未配置时保持 provider 默认行为
...
- 全新重构插件自带配置页视觉与交互体验
- 新增群聊/私聊分区列表、头像展示、策略状态高亮
- 群聊策略显示为 `整群放行` / `群组细化`
- 私聊策略显示为 `私聊放行` / `私聊关闭`
- 新增浅色、深色、自动主题切换并持久化保存
- 新增系统信息、Python 版本、AstrBot 版本展示
- 优化群聊和私聊配置表单，支持保存后热刷新
- 清理无效缓存与冗余代码，修复前端初始化顺序
- 优化天气接口调用，避免阻塞后端异步路由
